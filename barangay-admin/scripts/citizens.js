...
window.renderCitizens = function (DATA) {
  ...
  const filtered = () => {
    ...
    if (state.q) {
      ...
      rows = rows.filter(r =>
        (r.name + ' ' + (r.address || '') + ' ' + (r.contact || '')).toLowerCase().includes(q)
      );
    }
  ...
  const draw = () => {
    ...
    document.getElementById('countLbl').textContent = total
      ? `${start + 1}-${Math.min(start + state.per, total)} of ${total}`
      : '0 of 0';
    document.getElementById('prev').disabled = state.page === 0;
    document.getElementById('next').disabled = start + state.per >= total;
    document.getElementById('rows').innerHTML = slice
      .map(
        r => `
      <tr>
        <td>${r.id}</td>
        ...
        <td>${r.contact || ''}</td>
        <td class="text-end">
          <button class="btn btn-sm btn-outline-primary" data-view="${r.id}">
  ...
  document.getElementById('formAdd').addEventListener('submit', e => {
    ...
    document.getElementById('toastMsg').textContent = 'Citizen added.';
    toast.show();
    state.page = 0;
    draw();
  });
  ...
  document.addEventListener('click', e => {
    ...
    document.getElementById('viewBody').innerHTML = `
      ...
      <div class="border rounded p-3 mb-2">
        ...
        <div>${c.address || '<span class="text-muted">—</span>'}</div>
      </div>
      <div class="border rounded p-3">
        <div class="small text-muted">Contact</div>
        <div>${c.contact || '<span class="text-muted">—</span>'}</div>
      </div>`;
